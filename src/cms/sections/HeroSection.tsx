...
export default function HeroSection({ section }: { section?: CMSSection }) {
  ...
  // Editorial Parallax & Fade-out across 200vh (Double Viewport Height)
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ['start start', 'end start']
  });

  const titleParallaxY = useTransform(scrollYProgress, [0, 1], [0, -180]);
  const metaParallaxY = useTransform(scrollYProgress, [0, 1], [0, -90]);
  const ctaParallaxY = useTransform(scrollYProgress, [0, 1], [0, -60]);
  const heroOpacity = useTransform(scrollYProgress, [0.3, 0.9], [1, 0]);
  ...
  return (
    <section ref={sectionRef} className="relative w-full h-[200vh] bg-black text-white font-sans select-none">
      {/* Sticky 100vh Viewport Container pinned while scrolling through 200vh */}
      <div className="sticky top-0 w-full h-screen overflow-hidden">
        ...
        {/* Hero Typography — Centered inside Sticky 100vh */}
        <motion.div
