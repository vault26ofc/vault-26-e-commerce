...
export default function HeroSection({ section }: { section?: CMSSection }) {
  ...
  // Parallax progress over 200vh hero container
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ['start start', 'end start']
  });

  // Background image subtle zoom over scroll
  ...
  return (
    <section ref={sectionRef} className="relative w-full h-[200vh] bg-black text-white font-sans select-none">
      {/* Background Image — Covers full 200vh scroll height */}
      <div className="absolute inset-0 w-full h-[200vh] overflow-hidden z-0">
        <motion.img
          ...
          className="w-full h-[200vh] object-cover object-center brightness-90"
        />
        {/* Vignette Overlay */}
        <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/10 to-black/60 pointer-events-none" />
      </div>

      {/* Hero Typography — Fixed dead center of screen for 200vh, unpins at next section */}
      <div className="sticky top-0 w-full h-screen flex flex-col items-center justify-center p-4 text-center z-10 pointer-events-none">
