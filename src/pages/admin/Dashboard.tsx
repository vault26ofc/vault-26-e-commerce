    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={k.l} className="border border-border p-5">
            <div className="eyebrow">{k.l}</div>
            <div className="font-display text-2xl mt-2">{k.v}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-8">
        <div className="border border-border p-5">
          ...
          <div className="h-56"><ResponsiveContainer><LineChart data={trend}><XAxis dataKey="d" fontSize={10} /><YAxis fontSize={10} /><Tooltip /><Line dataKey="orders" stroke="hsl(var(--accent))" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></div>
        </div>
        <div className="border border-border p-5">
          <div className="eyebrow mb-4">Revenue — last 30 days</div>
          <div className="h-56"><ResponsiveContainer><BarChart data={trend}><XAxis dataKey="d" fontSize={10} /><YAxis fontSize={10} /><Tooltip /><Bar dataKey="orders" fill="hsl(var(--foreground))" /></BarChart></ResponsiveContainer></div>
        </div>
      </div>

      <div className="border border-border">
        <div className="p-5 eyebrow border-b border-border">Recent orders</div>
          ...
          <tbody>
            {recent.map((o) => (
              <tr key={o.id} className="border-t border-border">
                ...
                <td className="p-3"><span className="text-[10px] uppercase tracking-widest bg-secondary px-2 py-1">{o.status}</span></td>
